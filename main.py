"""
一键运行脚本：顺序完成训练与评估（分类 + 认证）。
无命令行参数，默认数据路径 PalmBigDataBase，可按需改下面的配置常量。
"""

import torch
from torch.utils.data import DataLoader

from palm.config import load_cfg, set_seed
from palm.data import AuthDataset, build_transform
from palm.models import FeatureNet
from palm.trainers import (
    evaluate_authentication,
    train_auth_ce,
    train_auth_contrastive,
    train_classifier,
)
...
def run_all() -> None:
    ...
    # 2) 认证训练（对比损失）
    contrastive_result = train_auth_contrastive(
        cfg=cfg,
        data_root=DATA_ROOT,
